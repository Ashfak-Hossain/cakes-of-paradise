  ...
  const setOpen = React.useCallback(
    (value: boolean | ((value: boolean) => boolean)) => {
      ...
      if (setOpenProp) {
        ...
        dispatch(setIsSidebarOpen(openState));
      }
      dispatch(setIsSidebarOpen(!openState));
    },
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn(
            'group/sidebar-wrapper p-2 flex min-h-svh w-full has-[[data-variant=inset]]:bg-sidebar',
            className
          )}
          ref={ref}
          {...props}
...
>(
  (
    ...
    const isMobileSidebarOpen = useAppSelector(
      (state: RootState) => state.ui.isMobileSidebarOpen
    );
    ...
    if (isMobile) {
      return (
        <Sheet
          ...
          onOpenChange={() => dispatch(setIsMobileSidebarOpen(!isMobileSidebarOpen))}
          {...props}
        >
          <SheetContent
    ...
    return (
      ...
      >
        ...
        <div
          className={cn(
            'fixed inset-y-0 bg-sidebar z-10 hidden h-svh w-[--sidebar-width] transition-[left,right,width] duration-200 ease-linear md:flex',
            side === 'left'
              ? 'left-0 group-data-[collapsible=offcanvas]:left-[calc(var(--sidebar-width)*-1)]'
              : 'right-0 group-data-[collapsible=offcanvas]:right-[calc(var(--sidebar-width)*-1)]',
            // Adjust the padding for floating and inset variants.
            variant === 'floating' || variant === 'inset'
              ? 'p-2 pt-6 group-data-[collapsible=icon]:w-[calc(var(--sidebar-width-icon)_+_theme(spacing.4)_+2px)]'
